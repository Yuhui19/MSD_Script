...
static PTR(Expr) parse_expr(std::istream &in) {
  ...
  if (c == '=') {
      ...
      in >> c;
      if( c != '=')
          throw std::runtime_error((std::string) "Expected == after expression, not " + c);
...
//Parses content with undecided operation in '+' or '*' from 'in'.
static PTR(Expr) parse_inner(std::istream &in) {
  PTR(Expr) e;
  ...
  if (c == '(') {
      ...
      if (c == ')')
          ...
          throw std::runtime_error("expected an end parenthesis");
  } else if (isdigit(c) || c == '-') {
      e = parse_number(in);
  } else if (isalpha(c)) {
      e = parse_variable(in);
      ...
      if (keyword == "_true")
          ...
          throw std::runtime_error((std::string)"unknown input: " + keyword);
  } else {
      throw std::runtime_error((std::string)"unexpected input: " + c);
  }
...
// Allow to run no matter has whitespace or not
static char peek_after_spaces(std::istream &in) {
  char c;
  while (1) {
    c = in.peek();
...
TEST_CASE( "Simple expressions" ) {
    // Single number
    CHECK ( parse_str_error(" ( 1 ") == "expected an end parenthesis" );
    ...
    CHECK( parse_str(" 11 * ( 10 + 1 ) ")->equals(NEW(MultExpr)(NEW(NumExpr)(11), ten_plus_one)) );
    ...
    // Invalid keywords
    CHECK( parse_str_error("_maybe ") == "unknown input: _maybe" );
    
    // Invalid chars
    CHECK ( parse_str_error("!") == "unexpected input: !" );
}
...
TEST_CASE( "FunExpr & CallFunExpr " ) {
    ...
    CHECK( parse_str("_let f = _fun(x) x + x"
                     "_in f(2)")
          ->to_value(Env::emptyenv)->equals(NEW(NumVal)(4)) );
    ...
    CHECK( parse_str("_let f = _fun(x)"
                     ...
                     "x * x + y * y"
                     "_in f(2)(3)")
          ->to_value(Env::emptyenv)->equals(NEW(NumVal)(13)) );
    ...
    CHECK( parse_str("_let add = _fun(x)"
                     ...
                     "_in addFive(10)")
          ->to_value(Env::emptyenv)->to_string() == "15" );
    ...
    CHECK( parse_str("_let factrl = _fun(factrl)"
                     ...
                     "_in factrl(factrl)(5)")
          ->to_value(Env::emptyenv)->to_string() == "120" );
    ...
    CHECK( parse_str("_let fib = _fun (fib)"
                     ...
                     "                 _else fib(fib)(x + -1)"
                     "                       + fib(fib)(x + -2)"
                     "_in fib(fib)(10)")->to_value(Env::emptyenv)->to_string() == "89");
